...
function bg_section($text, $sourcetitle, $mdwiki_revid)
{
    ...
    preg_match('/\{\{\s*Превод\s*от\s*\|/iu', $text, $ma);
    // ---
    if (!empty($ma)) {
        return $text;
    }
    ...
    if (preg_match('/\[\[(Категория|Category):/iu', $text, $m, PREG_OFFSET_CAPTURE)) {
        $pos = $m[0][1];
...
function bg_fixes($text, $sourcetitle, $mdwiki_revid)
{
    ...
    $text = preg_replace('/\[\[\s*(Категория|Category)\s*:\s*Translated from MDWiki\s*\]\]/iu', '', $text);
    // ---
    return $text;
}
