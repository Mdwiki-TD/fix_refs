...
use function WpRefs\MovesDots\remove_spaces_between_last_word_and_beginning_of_ref;
...
class mv_dots_afterTest extends TestCase
{
    ...
    {
        $input = <<<'TXT'
            Article text <ref>{{Citar web|Text|author=John|language=en}}</ref> [[Հիպոկրատ|Հիպոկրատի]] կողմից <ref name="Os2018" /><ref>{{ref|<!!>}}</ref>։
            == test ==
            TXT;
        $expected = <<<'TXT'
            Article text <ref>{{Citar web|Text|author=John|language=en}}</ref> [[Հիպոկրատ|Հիպոկրատի]] կողմից<ref name="Os2018" /><ref>{{ref|<!!>}}</ref>։
